...
public class GraphicsToGraphics2DAdaptor extends Graphics {
	...
	private void createSWTGraphics() {
		...
		image = new Image(Util.getInstance().getDisplay(), tempRect);
		GC gc = new GC(image);
	 ...
	 * 
	 * @see
	 * org.eclipse.draw2d.Graphics#clipRect(org.eclipse.draw2d.geometry.Rectangle
	 * )
	 */
	@Override
	...
	@Override
	public void drawTextLayout(TextLayout layout, int x, int y, int selectionStart, int selectionEnd, Color selectionForeground,
			Color selectionBackground) {
		checkState();
		if (!layout.getBounds().isEmpty()) {
			Image image = new Image(Util.getInstance().getDisplay(), layout.getBounds().width,
	 ...
	 * 
	 * @see
	 * org.eclipse.draw2d.Graphics#drawPolygon(org.eclipse.draw2d.geometry.PointList
	 * )
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.eclipse.draw2d.Graphics#fillPolygon(org.eclipse.draw2d.geometry.PointList
	 * )
	 */
	@Override
	...
	public void drawString(String s, int x, int y) {
		...
		if (paintNotCompatibleStringsAsBitmaps && Math.abs(swtStringSize.width - stringLength) > 2) {
			...
			Image image = new Image(Util.getInstance().getDisplay(), swtStringSize.width,
					swtStringSize.height);
			GC gc = new GC(image);
			gc.setForeground(getForegroundColor());
			gc.setBackground(getBackgroundColor());
			gc.setAntialias(getAntialias());
	...
	public void setFont(Font f) {
		...
		if (fontInfo[0] != null) {
			...
			float fsize = height * (float) Util.getInstance().getDisplay().getDPI().x / 72.0f;
	...
	private GeneralPath createPathAWT(Path path) {
		...
		for (int i = 0; i < pathData.types.length; i++) {
			...
			case SWT.PATH_CUBIC_TO:
				pathAWT.curveTo(pathData.points[idx++] + transX, pathData.points[idx++] + transY, pathData.points[idx++] + transX,
						pathData.points[idx++] + transY, pathData.points[idx++] + transX, pathData.points[idx++] + transY);
				break;
			case SWT.PATH_QUAD_TO:
				pathAWT.quadTo(pathData.points[idx++] + transX, pathData.points[idx++] + transY, pathData.points[idx++] + transX,
	 ...
	 * 
	 * @see
	 * org.eclipse.gmf.runtime.draw2d.ui.render.awt.internal.DrawableRenderedImage
	 * #allowDelayRender()
	 */
	...
	private Stroke createStroke() {
		 ...
		 * that below.
		 */
		stroke = new BasicStroke(currentState.lineAttributes.width != 0 ? currentState.lineAttributes.width : 1, awt_cap, awt_join,
				currentState.lineAttributes.miterLimit, awt_dash, currentState.lineAttributes.dashOffset);
		return stroke;
	}
