 ...
 */
public class FixedThumbnail extends Figure implements UpdateListener {
	...
	class ThumbnailUpdater implements Runnable {
		...
		public void resetTileValues() {
			...
			tileSize = new Dimension((int) Math.ceil((float) getSourceRectangle().width / (float) hTiles), (int) Math
					.ceil((float) getSourceRectangle().height / (float) vTiles));
		...
		public void run() {
			...
			// CHANGED START
			// do not paint multiple tiles, instead just draw the complete
			// rectangle once
		...
		public void start() {
			...
			thumbnailGraphics = new FixedScaledGraphics(new SWTGraphics(thumbnailGC));
			...
			setScales(targetSize.width / (float) getSourceRectangle().width, targetSize.height / (float) getSourceRectangle().height);
